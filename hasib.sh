...
read -p "Service name [hasib-vip]: " SERVICE_NAME
SERVICE_NAME=${SERVICE_NAME:-hasib-vip}
...
gcloud builds submit --tag "$IMAGE" --project="$PROJECT_ID" --timeout=20m --quiet
...
echo "$VLESS" > /tmp/hasib.vless

echo ""
echo -e "${CYAN}Generating Download Link...${RESET}"
DOWNLOAD_LINK=$(curl --upload-file /tmp/hasib.vless https://transfer.sh/hasib.vless 2>/dev/null)
